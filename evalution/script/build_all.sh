...
nvcc -O3 --generate-code=arch=compute_70,code=sm_70 -DBANK_OPTIMIZE -DREORDER_OUT_CHANNEL ${codebase_dir}spconv2d.cc -lcublas -lcudnn ${codebase_dir}build/cuda_utils.o ${codebase_dir}build/tensor_utils.o ${codebase_dir}build/spconv2d_utils.o ${codebase_dir}build/spconv2d_kernel.o -o ${codebase_dir}build/spconv2d_test.o
