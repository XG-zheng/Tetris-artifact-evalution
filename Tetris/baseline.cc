...
void SparseConv2d_Baseline(float *h_input, float *h_filter, float *h_output,
  ...
  for (int i = 0; i < WARM; ++i) {
    Baseline<ValueType, OffsetType, PositionType,
                        3, 1, 128, 2, 4>(d_input, d_offsets,
                          d_position, d_values, d_output,
                          batch_size, img_h, img_w, in_channel,
                          out_channel, kernel_size, stride,
                          padding, cuda_stream);
  }
  cudaDeviceSynchronize();
  ...
  elapsedTime /= REPEAT;
  std::cout<<"Time of Baseline :"<< elapsedTime <<"ms"<<"\n";
  std::cout<<"FLOPS of Baseline :"<< (long long)out_h * out_w * batch_size * out_channel * in_channel * kernel_size * kernel_size * 2 * 1000/(elapsedTime)/1024/1024/1024/1024 << "T\n";
...
int main(int argc,char *argv[]) {
  ...
  SparseConv2d_Baseline<float, int, int>(h_input, h_filter, h_sparse, batch_size, img_h, img_w,
